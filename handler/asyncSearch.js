...
exports.search = function(requestBookName, callback) {
	//console.log(urlStr);
    ...
    if(config.isUseCache && existsBookIndex(requestBookName)) {
    	...
    	robot_Index(requestBookName, bookIndexUrl, callback);
    	return;
    }
	...
	http.get(urlStr, function(res) {
		...
		res.on('end', function(chunk) {
			...
			robot_Index(bookName, bookIndexUrl, callback);
		});
	});
};

function robot_Index(bookName, indexUrl, callback) {
	http.get(indexUrl, function(res) {
		...
		res.on('end', function(chunk) {
			...
			if(arrUrls.length > 0)
				GetContent(arrUrls, bookName, callback);
			else
				callback();
		});
	});
...
function GetContent(arrUrls, bookName, callback, errTimes) {
	if(!errTimes) errTimes = 0;
	if(arrUrls.length == 0) { console.log('robot ' + bookName + ' end\r\n'); callback(); return; } 
	//if(arrUrls.length < 100) { console.log('robot end'); callback(); return; } 	//测试时减下载量

	if(existsChapter(bookName, arrUrls[0].name)) {
		//console.log('..has context' + arrUrls[0].name);
		arrUrls = arrUrls.slice(1, arrUrls.length);
    	GetContent(arrUrls, bookName, callback, errTimes);
    	return;
	}
	...
	http.get(arrUrls[0].url, function(res) {
		...
		res.on('end', function(chunk) {
        	...
        	//进行下一章
        	arrUrls = arrUrls.slice(1, arrUrls.length);
    		setTimeout(function() { GetContent(arrUrls, bookName, callback, errTimes) }, 1000 * config.nextMinite);       
		});
